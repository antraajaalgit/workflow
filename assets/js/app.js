...
const SESSION_KEY = 'nagare_session';
...
function viewTeam(){
  ...
  const rows = team.map(u=>{
    ...
    return `<div class="row-item">
      ${avatar(u)}
      <div style="flex:1"><b style="font-size:14px">${esc(u.name)}</b><div class="muted small">${u.dept} Department</div></div>
  ...
  return `<p class="muted" style="margin-bottom:16px">Each person owns their queue — accountability is explicit. Load balancing follows Heijunka (new work goes to the least-loaded person in the right department).</p><div class="list">${rows}</div>`;
}
...
function bindView(){
  ...
  const rd=$('#reset-data'); if(rd) rd.onclick=()=>{ Store.reset(); toast('🧹 Demo data reset'); render(); buildNav(); };
