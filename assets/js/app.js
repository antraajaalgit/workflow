...
function viewTasks(){
  ...
  const visible=session.role==='team'?S().tasks.filter(t=>t.ownerId===session.id):adminMyTasks?S().tasks.filter(t=>(t.ownerIds?.length?t.ownerIds:(t.ownerId?[t.ownerId]:[])).includes(session.id)):S().tasks;
